...
def generate_numbers2():
    """ GENERATE one object/number at a time = Lazy List
        using a return and generator comprehension, eek!
        Generator Comprehensions use () brackets instead of []
        like List comprehensions
        """
    print("Executing generate numbers2()...")
    return (x for x in range(0, 1000000) if x % 2 == 0)


def list_vs_gen_comprehensions():
    # Example - list versus gen comprehensions:
    nums_squared_lc = [num ** 2 for num in range(10000)]
    nums_squared_gc = (num ** 2 for num in range(10000))
    print(f"{'List Comprehension Size':>30s}: {sys.getsizeof(nums_squared_lc):>8,.0f} bytes" )
    print(f"{'Generator Comprehension Size':>30s}: {sys.getsizeof(nums_squared_gc):>8,.0f} bytes")


def alternate_use_of_generator():
